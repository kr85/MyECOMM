<?php
    ...
    class phpmailerTest extends PHPUnit_Framework_TestCase {
        ...
        function setUp() {
            ...
            require_once $this->INCLUDE_DIR . 'PHPMailer.php';
            ...
            if (array_key_exists('mail_cc', $_REQUEST) and strlen(
                    $_REQUEST['mail_cc']
                ) > 0
            ) {
        ...
        function BuildBody() {
            ...
            if ($this->Mail->ContentType == "text/html" || strlen(
                    $this->Mail->AltBody
                ) > 0
            ) {
                $eol = "<br/>";
        ...
        function test_Html() {
            ...
            $this->Mail->Body = "This is a <b>test message</b> written in HTML. </br>" . "Go to <a href=\"http://code.google.com/a/apache-extras.org/p/phpmailer/\">" . "http://code.google.com/a/apache-extras.org/p/phpmailer/</a> for new versions of " . "phpmailer.  <p/> Thank you!";
        ...
        function test_AltBody_Attachment() {
            ...
            if (is_writable('.')) {
                file_put_contents(
                    'message.txt',
                    $this->Mail->CreateHeader() . $this->Mail->CreateBody()
                );
            } else {
                $this->assertTrue(
                    false,
                    'Could not write local file - check permissions'
                );
            }
        }
        ...
        function test_Addressing() {
            ...
            $this->assertFalse(
                $this->Mail->AddAddress('a@example.com'),
                'Duplicate addressing failed'
            );
            $this->assertTrue(
                $this->Mail->AddCC('b@example.com'),
                'CC addressing failed'
            );
            $this->assertFalse(
                $this->Mail->AddCC('b@example.com'),
                'CC duplicate addressing failed'
            );
            $this->assertFalse(
                $this->Mail->AddCC('a@example.com'),
                'CC duplicate addressing failed (2)'
            );
            $this->assertTrue(
                $this->Mail->AddBCC('c@example.com'),
                'BCC addressing failed'
            );
            $this->assertFalse(
                $this->Mail->AddBCC('c@example.com'),
                'BCC duplicate addressing failed'
            );
            $this->assertFalse(
                $this->Mail->AddBCC('a@example.com'),
                'BCC duplicate addressing failed (2)'
            );
            $this->assertTrue(
                $this->Mail->AddReplyTo('a@example.com'),
                'Replyto Addressing failed'
            );
            $this->assertFalse(
                $this->Mail->AddReplyTo('a@example..com'),
                'Invalid Replyto address accepted'
            );
        ...
        function test_Translations() {
            ...
            foreach (new DirectoryIterator('../language') as $fileInfo) {
                ...
                if (preg_match(
                    ...
                    $this->assertTrue(
                        empty($missing),
                        "Missing translations in $lang: " . implode(
                            ', ',
                            $missing
                        )
                    );
        ...
        function test_Miscellaneous() {
            ...
            $this->assertFalse(
                $this->Mail->set('x', 'y'),
                'Invalid property set succeeded'
            );
            $this->assertTrue(
                $this->Mail->set('Timeout', 11),
                'Valid property set failed'
            );
        }
    }
