...
*/
    ...
    class PHPMailer {
        ...
        public static function ValidateAddress($address) {
            if ((defined('PCRE_VERSION')) && (version_compare(
                        PCRE_VERSION,
                        '8.0'
                    ) >= 0)
            ) {
        ...
        public function PreSend() {
            try {
                $this->mailHeader = "";
                if ((count($this->to) + count($this->cc) + count(
                            $this->bcc
                        )) < 1
                ) {
                    throw new phpmailerException(
        ...
        protected function SmtpSend($header, $body) {
            ...
            if (!$this->smtp->Mail($smtp_from)) {
                $this->SetError(
                    $this->Lang('from_failed') . $smtp_from . " : " . implode(
                        ",",
                        $this->smtp->getError()
                    )
                );
        ...
        public function CreateHeader() {
            ...
            if ((($this->Mailer == 'sendmail') || ($this->Mailer == 'mail')) && (count(
                        $this->bcc
                    ) > 0)
            ) {
        ...
        public function AddAttachment(
            ...
            try {
                if (!@is_file($path)) {
                    throw new phpmailerException(
                        $this->Lang('file_access') . $path, self::STOP_CONTINUE
                    );
                }
        ...
        protected function EncodeFile($path, $encoding = 'base64') {
            try {
                if (!is_readable($path)) {
                    throw new phpmailerException(
                        $this->Lang('file_open') . $path, self::STOP_CONTINUE
                    );
                }
        ...
        public function EncodeHeader($str, $position = 'text') {
            ...
            if (strlen($str) / 3 < $x) {
                $encoding = 'B';
                if (function_exists('mb_strlen') && $this->HasMultiBytes(
                        $str
                    )
                ) {
        ...
        protected function SetError($msg) {
            ...
            if ($this->Mailer == 'smtp' and !is_null($this->smtp)) {
                ...
                if (!empty($lasterror) and array_key_exists(
                        'smtp_msg',
                        $lasterror
                    )
                ) {
        ...
        public function set($name, $value = '') {
            try {
                if (isset($this->$name)) {
                    ...
                    throw new phpmailerException(
                        $this->Lang('variable_set') . $name, self::STOP_CRITICAL
                    );
                }
            } catch (Exception $e) {
                $this->SetError($e->getMessage());
        ...
        public function DKIM_Add($headers_line, $subject, $body) {
            ...
            $DKIMb64 = base64_encode(
                pack("H*", sha1($body))
            ); // Base64 of packed binary SHA-1 hash of body
            $ident = ($this->DKIM_identity == '') ?
                '' :
        ...
        protected function doCallback(
            ...
            if (!empty($this->action_function) && is_callable(
